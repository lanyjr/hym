        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if x is None:
                    ...
                    z = 1 / t * p - (t-1)/t * lr * (d_p - (1 - beta) * n) 
                    state['x'].mul_((t-1)/t).add_(z)
                    # state['x'].mul_((t-1)/t).add_(p, alpha = 1 / t)
        ...
        for group in self.param_groups:
            if group["use_muon"]:
                for p in group["params"]:
                    ...
                    om = muon_update_wo_momentum_update(m)
                    on = muon_update_wo_momentum_update(n)
                    update = (beta  / (t+1)  * (1 - momentum ** t) / (1 - momentum) * om + (1 - beta) * on)
                    p.mul_(1 - group["lr"] * group["weight_decay"])
                    p.add_(update.reshape(p.shape), alpha=-group["lr"])
            else:
                # for p in group["params"]:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Corrected update
                update = (exp_avg / bias_correction1) / (denom / (bias_correction2 ** 0.5))
