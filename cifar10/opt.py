            ...
            for p in group["params"]:
                ...
                if momentum != 0.0:
                    ...
                    if m is None:
                        ...
                        m.mul_((t-1)/t).add_(d_p)
                        # n.mul_((t-1) / t).add_(d_p).mul_(momentum)
                        n.mul_((t-1) / t).mul_(momentum).add_(d_p)
...
class DM_AdamW(torch.optim.Optimizer):
    def __init__(
        self,
        params,
        lr=1e-3,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, beta = beta)
        super().__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                m.mul_((t-1)/t).add_(grad)
                n.mul_((t-1) / t).add_(grad).mul_(beta1)
                # m_t = beta1*m_{t-1} + (1-beta1)*g_t
                exp_avg = (beta  / (t+1)  * (1 - beta1 ** t) / (1 - beta1) * m + (1 - beta) * n) * (1 - beta1)
                # v_t = beta2*v_{t-1} + (1-beta2)*g_t^2
                exp_avg_sq.mul_(beta2).addcmul_(grad, grad, value=1 - beta2)
